...
public class Violation {
    ...
    private Debt debt;
    private PackageRecord packageRecord;

    public Violation(Debt debt, PackageRecord packageRecord) {
        this.debt = debt;
        this.packageRecord = packageRecord;
    }

    public Violation(Debt debt, ClassRecord classRecord) {
    ...
    public String toXML() {
        ...
        for (int count = 0; count < debt.count() ; count++) {
            xml += "<violation rule=\"" + debt.desc() + "\" ruleset=\"Debt\" package=\"" + packageRecord.getName() + "\"/>";
        }
        return xml;
    }
}
